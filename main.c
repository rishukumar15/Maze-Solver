       ...
       for(i=0;i<max;i++ )
        {
            while(!(IsQEmpty(&(pq.prq[i]))))
            {
                ...
                for(int k=0;k<4;k++)
                {
                    if(Isvalid(xx+rrow[k],yy+ccol[k],n,m,Maze)==1)
                    {
                         if( ( xx+rrow[k] != (var->prev)->row ) || ( yy+ccol[k] != (var->prev)->col ) )
                        {
                            vary->row=xx+rrow[k];
                            vary->col=yy+ccol[k];
                            vary->next=NULL;
                            vary->prev=var;
                            insrt(vary,i+Maze[xx+rrow[k]][yy+ccol[k]],&pq);
                            cnt++;
                        }
                    }
                }
